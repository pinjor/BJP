...
class MemberSearchScreen extends StatefulWidget {
  static const String name = '/membersearchlist';
...
class _MemberSearchScreenState extends State<MemberSearchScreen> {
  ...
  void _searchData() {
    setState(() {
      filteredData = data.where((member) {
        final userIdMatch = _userIdController.text.isEmpty ||
            member["userId"]!
                .toLowerCase()
                .contains(_userIdController.text.toLowerCase());
        ...
        final divisionMatch = _selectedDivision == null ||
  ...
  void _copyToClipboard(String mobile) async {
    ...
    await Clipboard.setData(ClipboardData(text: mobile)).then((_) async {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("মোবাইল নম্বর কপি হয়েছে!")),
      );
      ...
      if (await canLaunchUrl(phoneUri)) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("ডায়ালার খুলতে ব্যর্থ!")),
        );
      }
    }).catchError((error) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("কপি করতে ব্যর্থ: $error")),
      );
    });
  }
        ...
        child: Column(
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: ['ঢাকা', 'চট্টগ্রাম', 'রাজশাহী', 'বরিশাল']
                                  ...
                                  ElevatedButton(
                                    ...
                                    style: ElevatedButton.styleFrom(
                                      ...
                                      padding: EdgeInsets.symmetric(
                                          horizontal: 8,
                                          vertical: 4), // Reduce padding
                                    ),
                                    child: Text("নম্বর কপি করুন",
                                        style: TextStyle(fontSize: 12)),
                                  ),
                                  SizedBox(
                                      height:
                                          4), // Reduce spacing between buttons
                                  ElevatedButton(
                                    onPressed: () {
